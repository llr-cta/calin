...
private:
    Vec64cb(int b); // Prevent constructing from int, etc.
    Vec64cb & operator = (int x); // Prevent assigning int because of ambiguity
};
...
static inline int horizontal_find_first(Vec64cb const & a) {
    int f = horizontal_find_first(a.get_low());
    if (f < 0) {
        f = horizontal_find_first(a.get_high());
        if (f >= 0) f += 32;
    }
    return f;
}
...
static inline uint32_t horizontal_count(Vec64cb const & a) {
    return horizontal_count(a.get_low()) + horizontal_count(a.get_high());
}
...
private:
    Vec32sb(int b); // Prevent constructing from int, etc.
    Vec32sb & operator = (int x); // Prevent assigning int because of ambiguity
};
...
// Get index to the first element that is true. Return -1 if all are false
static inline int horizontal_find_first(Vec32sb const & a) {
    int f = horizontal_find_first(a.get_low());
    if (f < 0) {
        f = horizontal_find_first(a.get_high());
        if (f >= 0) f += 16;
...
// Count number of true elements
static inline uint32_t horizontal_count(Vec32sb const & a) {
    return horizontal_count(a.get_low()) + horizontal_count(a.get_high());
}
...
* implementation dependent and may be different for different instruction sets.
* An index out of range does not produce an error message or exception.
*
* Example:
...
* Vec8i c = lookup8(a,b);                   // c is       (102, 100, 100, 106, 104, 103, 105, 100)
*
*****************************************************************************/
