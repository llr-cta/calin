...
void calin::util::utm::geographic_to_tm_sphere(double R, double k0,
...
void calin::util::utm::tm_to_geographic_sphere(double R, double k0,
			     double lon_mer, double FN, double FE,
			     double N, double E,
			     double& lat_rad, double& lon_rad)
{
...
void calin::util::utm::geographic_to_ps_sphere(double R, double k0,
...
void calin::util::utm::ps_to_geographic_sphere(double R, double k0,
			     Hemisphere hemi, double FN, double FE,
			     double N, double E,
			     double& lat_rad, double& lon_rad)
{
...
void calin::util::utm::geographic_to_ps(double a, double e2, double k0,
		      Hemisphere hemi, double FN, double FE,
		      double lat_rad, double lon_rad,
		      double& N, double& E)
{
...
void calin::util::utm::ps_to_geographic(double a, double e2, double k0,
		      Hemisphere hemi, double FN, double FE,
		      double N, double E,
		      double& lat_rad, double& lon_rad)
{
...
bool calin::util::utm::geographic_to_grid(double a, double e2,
		       double lat_rad, double lon_rad,
...
bool calin::util::utm::dmsStringToRad(const std::string& str, double& rad)
{
  unsigned degs=0;
  unsigned mins=0;
  unsigned secs=0;
...
std::string calin::util::utm::radToDMSString(double rad,
   unsigned sec_digits, bool dmsSep)
{
...
namespace {
	...
	void write_entry(double lat, double lon, double N, double E)
	{
	  LOG(INFO) << "LAT: " << radToDMSString(lat) << "   LONG: "
...
void calin::util::utm::test_ellipse()
{
  double a = 6378388.0;
  ...
  LOG(INFO) << "Tests transformations to/from UTM grid (reproduces Table 2-11 of DMTAM 8358.2)\n\n";
  ...
  write_entry(lat_rad, lon_rad, N, E);
  ...
  LOG(INFO) << "Tests transformations to/from UPS grid (reproduces Table 3-7 of DMTAM 8358.2)\n\n";
...
void calin::util::utm::test_sphere()
{
  for(unsigned ilat=0;ilat<10;ilat++)
  {
    ...
    for(unsigned ilon=0;ilon<10;ilon++)
		{
		  if(ilon)logger << ' ';
		  logger << std::fixed << std::setw(7) << std::setprecision(5)
			 	<< x[ilon];
		}
    logger << '\n';
