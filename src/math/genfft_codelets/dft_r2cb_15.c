/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cb.native -n 15 -standalone -fma -generic-arith -compact -name dft_codelet_r2cb_15 */
  ...
  DK(KP2_000000000, +2.000000000000000000000000000000000000000000000);
  {
    ...
    for (i = v; i > 0; i = i - 1, R0 = R0 + ovs, R1 = R1 + ovs, Cr = Cr + ivs, Ci = Ci + ivs, MAKE_VOLATILE_STRIDE(60, rs), MAKE_VOLATILE_STRIDE(60, csr), MAKE_VOLATILE_STRIDE(60, csi)) {
      ...
      }
      {
        ...
        Tm = FNMS(KP1_732050807, Tl, Ti);
        Tu = FMA(KP1_732050807, Tl, Ti);
      ...
      R0[0] = FMA(KP2_000000000, Te, T3);
      {
        ...
        R0[WS(rs, 6)] = FNMS(KP1_902113032, T10, TX);
        R0[WS(rs, 3)] = FMA(KP1_902113032, T12, T11);
        R1[WS(rs, 1)] = FMA(KP1_902113032, T10, TX);
        R1[WS(rs, 4)] = FNMS(KP1_902113032, T12, T11);
      }
      {
        E Ty, Ts, Tx, TK, TM, TE, TJ, TL, Tz;
        Ty = SUB(Tr, Tm);
        Ts = ADD(Tm, Tr);
        Tx = FMA(KP250000000, Ts, Th);
        TE = FMA(KP866025403, TD, TC);
        TJ = FMA(KP866025403, TI, TH);
        TK = FMA(KP618033988, TJ, TE);
        TM = FNMS(KP618033988, TE, TJ);
        R0[WS(rs, 5)] = SUB(Th, Ts);
        TL = FNMS(KP559016994, Ty, Tx);
        R1[WS(rs, 3)] = FNMS(KP1_902113032, TM, TL);
        R1[WS(rs, 6)] = FMA(KP1_902113032, TM, TL);
        Tz = FMA(KP559016994, Ty, Tx);
        R0[WS(rs, 2)] = FNMS(KP1_902113032, TK, Tz);
        R1[0] = FMA(KP1_902113032, TK, Tz);
      }
      {
        E TO, Tw, TN, TS, TU, TQ, TR, TT, TP;
        TO = SUB(Tv, Tu);
        Tw = ADD(Tu, Tv);
        TN = FMA(KP250000000, Tw, Tt);
        TQ = FNMS(KP866025403, TI, TH);
        TR = FNMS(KP866025403, TD, TC);
        TS = FNMS(KP618033988, TR, TQ);
        TU = FMA(KP618033988, TQ, TR);
        R1[WS(rs, 2)] = SUB(Tt, Tw);
        TT = FMA(KP559016994, TO, TN);
        R0[WS(rs, 7)] = FNMS(KP1_902113032, TU, TT);
        R1[WS(rs, 5)] = FMA(KP1_902113032, TU, TT);
        TP = FNMS(KP559016994, TO, TN);
        R0[WS(rs, 1)] = FNMS(KP1_902113032, TS, TP);
        R0[WS(rs, 4)] = FMA(KP1_902113032, TS, TP);
      }
    }
  }
}
