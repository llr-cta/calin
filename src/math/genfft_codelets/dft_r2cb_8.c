/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cb.native -n 8 -standalone -fma -generic-arith -compact -name dft_codelet_r2cb_8 */

/*
 * This function contains 20 FP additions, 12 FP multiplications,
 * (or, 8 additions, 0 multiplications, 12 fused multiply/add),
 * 19 stack variables, 2 constants, and 16 memory accesses
 */
void dft_codelet_r2cb_8(R * R0, R * R1, R * Cr, R * Ci, stride rs, stride csr, stride csi, INT v, INT ivs, INT ovs)
{
  DK(KP1_414213562, +1.414213562373095048801688724209698078569671875);
  DK(KP2_000000000, +2.000000000000000000000000000000000000000000000);
  {
    INT i;
    for (i = v; i > 0; i = i - 1, R0 = R0 + ovs, R1 = R1 + ovs, Cr = Cr + ivs, Ci = Ci + ivs, MAKE_VOLATILE_STRIDE(32, rs), MAKE_VOLATILE_STRIDE(32, csr), MAKE_VOLATILE_STRIDE(32, csi)) {
      E T4, Ta, T3, T9, T8, Tc, Tf, Tk, T1, T2, T5, Tj;
      T4 = Cr[WS(csr, 2)];
      Ta = Ci[WS(csi, 2)];
      T1 = Cr[0];
      T2 = Cr[WS(csr, 4)];
      T3 = ADD(T1, T2);
      T9 = SUB(T1, T2);
      {
        E T6, T7, Td, Te;
        T6 = Cr[WS(csr, 1)];
        T7 = Cr[WS(csr, 3)];
        T8 = ADD(T6, T7);
        Tc = SUB(T6, T7);
        Td = Ci[WS(csi, 1)];
        Te = Ci[WS(csi, 3)];
        Tf = ADD(Td, Te);
        Tk = SUB(Td, Te);
      }
      T5 = FMA(KP2_000000000, T4, T3);
      R0[WS(rs, 2)] = FNMS(KP2_000000000, T8, T5);
      R0[0] = FMA(KP2_000000000, T8, T5);
      Tj = FNMS(KP2_000000000, T4, T3);
      R0[WS(rs, 3)] = FNMS(KP2_000000000, Tk, Tj);
      R0[WS(rs, 1)] = FMA(KP2_000000000, Tk, Tj);
      {
        E Tb, Tg, Th, Ti;
        Tb = FMA(KP2_000000000, Ta, T9);
        Tg = ADD(Tc, Tf);
        R1[WS(rs, 2)] = FNMS(KP1_414213562, Tg, Tb);
        R1[0] = FMA(KP1_414213562, Tg, Tb);
        Th = FNMS(KP2_000000000, Ta, T9);
        Ti = SUB(Tf, Tc);
        R1[WS(rs, 3)] = FNMS(KP1_414213562, Ti, Th);
        R1[WS(rs, 1)] = FMA(KP1_414213562, Ti, Th);
      }
    }
  }
}
