/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cb.native -n 24 -standalone -fma -generic-arith -compact -name dft_codelet_r2cb_24 */
...
void dft_codelet_r2cb_24(R * R0, R * R1, R * Cr, R * Ci, stride rs, stride csr, stride csi, INT v, INT ivs, INT ovs)
{
  ...
  DK(KP2_000000000, +2.000000000000000000000000000000000000000000000);
  {
    ...
    for (i = v; i > 0; i = i - 1, R0 = R0 + ovs, R1 = R1 + ovs, Cr = Cr + ivs, Ci = Ci + ivs, MAKE_VOLATILE_STRIDE(96, rs), MAKE_VOLATILE_STRIDE(96, csr), MAKE_VOLATILE_STRIDE(96, csi)) {
      E Tc, TZ, Tw, Tz, T1w, TY, T7, Tr, T1v, TO, TN, Tu, Ti, T1y, TG;
      E TS, T17, T1n, Tn, T1z, TL, TT, T1c, T1o;
      {
        E T8, T9, Ta, Tb;
        T8 = Cr[WS(csr, 6)];
        T9 = Cr[WS(csr, 10)];
      ...
      }
      {
        ...
        T1n = FNMS(KP1_732050807, T16, T15);
      }
      {
        ...
        T1o = FNMS(KP1_732050807, T1b, T1a);
      }
      {
        E Td, To, T1F, T1G;
        ...
        T1G = SUB(T1y, T1z);
        R0[WS(rs, 3)] = FNMS(KP2_000000000, T1G, T1F);
        R0[WS(rs, 9)] = FMA(KP2_000000000, T1G, T1F);
        {
          E T1x, T1D, T1C, T1E, T1A, T1B;
          ...
          T1C = SUB(T1A, T1B);
          T1E = ADD(T1B, T1A);
          R1[WS(rs, 7)] = FNMS(KP1_414213562, T1C, T1x);
          R1[WS(rs, 4)] = FMA(KP1_414213562, T1E, T1D);
          R1[WS(rs, 1)] = FMA(KP1_414213562, T1C, T1x);
          R1[WS(rs, 10)] = FNMS(KP1_414213562, T1E, T1D);
        }
      }
      {
        E TU, T1u, TR, T1t, TP, TQ;
        TU = ADD(TS, TT);
        T1u = ADD(T1n, T1o);
        TP = ADD(TN, TO);
        TQ = FMA(KP1_732050807, Tz, Tw);
        TR = SUB(TP, TQ);
        T1t = ADD(TP, TQ);
        R0[WS(rs, 10)] = ADD(TR, TU);
        R0[WS(rs, 1)] = ADD(T1t, T1u);
        R0[WS(rs, 4)] = SUB(TR, TU);
        R0[WS(rs, 7)] = SUB(T1t, T1u);
      }
      {
        E TM, T1i, TB, T1h, Tv, TA;
        TM = ADD(TG, TL);
        T1i = ADD(T17, T1c);
        Tv = ADD(Tr, Tu);
        TA = FNMS(KP1_732050807, Tz, Tw);
        TB = SUB(Tv, TA);
        T1h = ADD(Tv, TA);
        R0[WS(rs, 2)] = ADD(TB, TM);
        R0[WS(rs, 5)] = ADD(T1h, T1i);
        R0[WS(rs, 8)] = SUB(TB, TM);
        R0[WS(rs, 11)] = SUB(T1h, T1i);
      }
      {
        E T1l, T1r, T1q, T1s;
        {
        ...
        R1[WS(rs, 2)] = FNMS(KP707106781, T1q, T1l);
        R1[WS(rs, 5)] = FMA(KP707106781, T1s, T1r);
        R1[WS(rs, 8)] = FMA(KP707106781, T1q, T1l);
        R1[WS(rs, 11)] = FNMS(KP707106781, T1s, T1r);
      }
      {
        E T11, T1f, T1e, T1g;
        {
        ...
        R1[WS(rs, 9)] = FNMS(KP707106781, T1e, T11);
        R1[0] = FMA(KP707106781, T1g, T1f);
        R1[WS(rs, 3)] = FMA(KP707106781, T1e, T11);
        R1[WS(rs, 6)] = FNMS(KP707106781, T1g, T1f);
      }
    }
  }
}
