...
visit_telescope_run(const TelescopeRunConfiguration* run_config)
{
  ...
  ped_iir_old_ = std::max(std::min(config_.pedestal_filter_constant(), 0.0f), 1.0f);
  ped_iir_new_ = (1.0f - ped_iir_old_)/float(nsamp_);
  ...
  all_sig_.resize(nchan_);
  all_mean_t_.resize(nchan_);
...
visit_telescope_event(uint64_t seq_index, TelescopeEvent* event)
{
  ...
  analyze_waveforms(data, nchan_, nsamp_, chan_ped_est_.data(), ped_iir_old_, ped_iir_new_,
    chan_max_index_.data(), chan_max_.data(),
    chan_bkg_win_sum_.data(), chan_sig_win_sum_.data(),
    chan_all_sum_q_.data(), chan_all_sum_qt_.data(), all_sig_.data(), all_mean_t_.data());
  return true;
}
