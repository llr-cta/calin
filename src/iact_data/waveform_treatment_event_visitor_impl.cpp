...
avx2_analyze_waveforms(const uint16_t*__restrict__ data)
{
  ...
  __m256 mean_t_c1 = _mm256_set1_ps(float(nsamp_*(nsamp_-1)/2)/float(window_n_));
  __m256 mean_t_c2 = _mm256_set1_ps(float(nsamp_)/float(window_n_));
  ...
  for(unsigned iblock=0;iblock<nblock;iblock++)
  {
    ...
    for(isamp=window_n_;isamp<nsamp_;++isamp)
    {
      int iss = isamp-16;
      ...
      __m256i mask = _mm256_cmpeq_epi32(*(__m256i*)(sig_window_0_ + iblock*16), visamp);
    ...
    __m256 ped_l = _mm256_load_ps(chan_ped_est_+iresvec);
    ...
    _mm256_store_ps(chan_ped_est_+iresvec, ped_l);

    __m256 ped_u = _mm256_load_ps(chan_ped_est_+iresvec+1);
    ...
    _mm256_store_ps(chan_ped_est_+iresvec+1, ped_u);
    ...
    _mm256_store_ps(chan_sig_+iresvec, _mm256_sub_ps(_mm256_cvtepi32_ps(sig_l), ped_l));
    _mm256_store_ps(chan_sig_+iresvec+1, _mm256_sub_ps(_mm256_cvtepi32_ps(sig_u), ped_u));
    ...
    _mm256_store_ps(chan_mean_t_+iresvec, _mm256_div_ps(nom, denom));
    ...
    _mm256_store_ps(chan_mean_t_+iresvec+1, _mm256_div_ps(nom, denom));
